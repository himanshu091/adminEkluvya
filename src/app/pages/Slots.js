...
import { createChapter, createSlots, createSubject, fetchChapterById, fetchSubjectById } from '../../api'
import ChapterItem from '../Component/ChapterItem'
import SlotItem from '../Component/SlotItem'

function Slots({match}) {
    const [data, setdata] = useState(null)
    const [showForm, setshowForm] = useState(false)
    const dateQ = useRef(null)
    const durationQ = useRef(null)
    const stQ = useRef(null)
    const etQ = useRef(null)
    useEffect(() => {
    ...
    const fetchSlots = async () =>{
      const res = await fetchChapterById(match.params.chapterId)
      setdata(res)
   }
    const handleSubmit = async () =>{
      // chapterId, date, classId, duration, startTime, endTime
      const res = await createSlots(match.params.chapterId, dateQ.current.value, data.classId, durationQ.current.value, stQ.current.value, etQ.current.value)
      console.log(res)
      setshowForm(false)
      fetchSlots()
    ...
    <div className="card card-custom gutter-b">
       <div className="card-header">
          <div className="card-title"> 
             <h3 className="card-label" style={{textTransform: 'capitalize'}}>Chapter - {match.params.name}</h3>
          </div>
          <div className="card-toolbar"><button onClick={()=>{setshowForm(true)}} type="button" className="btn btn-primary">New Slot</button></div>
       </div>
       <div className="card-header">
       <p><strong>No. Of Session :</strong>{data && data.noOfSessions}</p>
       </div>
       <div className="card-body">
          ...
          <div className="react-bootstrap-table table-responsive">
             <table className="table table table-head-custom table-vertical-center overflow-hidden">
                ...
                <tbody>
                  {data && data.listOfSlotsId.map((slotItem, index) => {
                     if(slotItem.slotsId){
                        return <SlotItem fetchSlots={fetchSlots} key={slotItem.slotsId._id} data={slotItem.slotsId} index={index}/>
                     }
                  })}
                </tbody>
             </table>
    ...
    {showForm && <div style={{display:'block', position: 'absolute', top: '50px', zIndex:'100'}}><div role="document" className="modal-dialog modal-lg">
        <div className="modal-content">
          <div className="modal-header">
            <div className="modal-title h4" id="example-modal-sizes-title-lg">Add New Chapter</div>
            ...
            <form action="#" className="form form-label-right">
              <div className="form-group row">
                <div className="col-lg-6">
                  <label>Date</label><input type="date" ref={dateQ} className="form-control" name="className" placeholder="Date"  />
                  {/* <div className="feedback">Please enter <b>Class Name</b></div> */}
                </div>
                <div className="col-lg-6">
                  <label>Duration</label><input type="number" ref={durationQ} className="form-control" name="Language" placeholder="Duration in mins"  />
                  {/* <div className="feedback">Please enter <b>Per Session Cost</b></div> */}
                </div>
                <div className="col-lg-6">
                  <label>Start Time</label><input type="time" ref={stQ} className="form-control" name="Language" placeholder="Start Time"  />
                  {/* <div className="feedback">Please enter <b>Per Session Cost</b></div> */}
                </div>
                <div className="col-lg-6">
                  <label>End Time</label><input type="time" ref={etQ} className="form-control" name="Language" placeholder="End Time"  />
                  {/* <div className="feedback">Please enter <b>Per Session Cost</b></div> */}
                </div>
              </div>
            </form>
