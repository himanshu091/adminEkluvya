...
import { createChapter, createSubject, fetchSubjectById } from '../../api'
...
function Chapters({match}) {
    ...
    const noOfSessionsQ = useRef(null)
    useEffect(() => {
    ...
    const fetchChapters = async () =>{
      const res = await fetchSubjectById(match.params.subjectId)
      setdata(res)
   }
    const handleSubmit = async () =>{
      // (name, classId, subjectId, duration, price, noOfSessions)
      const res = await createChapter(nameQ.current.value, data.classId, match.params.subjectId, "60", "100", noOfSessionsQ.current.value)
      console.log(res)
      setshowForm(false)
      fetchChapters()
    ...
    <div className="card card-custom gutter-b">
       <div className="card-header">
          <div className="card-title">
             <h3 className="card-label" style={{textTransform: 'capitalize'}}>Subject - {match.params.name}</h3>
          </div>
          <div className="card-toolbar"><button onClick={()=>{setshowForm(true)}} type="button" className="btn btn-primary">New Chapter</button></div>
       </div>
       <div className="card-header">
       <p><strong>Description :</strong>{data && data.subjectDescription}</p>
       </div>
       <div className="card-body">
          ...
          <div className="react-bootstrap-table table-responsive">
             <table className="table table table-head-custom table-vertical-center overflow-hidden">
                ...
                <tbody>
                  {data && data.listOfChaptersId.map((chapterItem, index) => {
                     if(chapterItem.chapterId){
                        return <ChapterItem fetchChapters={fetchChapters} key={chapterItem.chapterId._id} data={chapterItem.chapterId} index={index}/>
                     }
                  })}
                </tbody>
             </table>
            ...
            <form action="#" className="form form-label-right">
              <div className="form-group row">
                <div className="col-lg-6">
                  <label>Chapter Name</label><input type="text" ref={nameQ} className="form-control" name="className" placeholder="Chapter Name"  />
                  {/* <div className="feedback">Please enter <b>Class Name</b></div> */}
                </div>
                <div className="col-lg-6">
                  <label>Total Sessions</label><input type="number" ref={noOfSessionsQ} className="form-control" name="Language" placeholder="No. of Sessions"  />
