...
import { fetchClasses, createClass, fetchClassById, createSubject } from '../../api'
...
function Subjects({match}) {
    ...
    const fetchSubjects = async () =>{
      const res = await fetchClassById(match.params.classId)
      setdata(res)
   }
    const handleSubmit = async () =>{
      // (name, description, language, classId)
      const res = await createSubject(nameQ.current.value, descQ.current.value, langQ.current.value, match.params.classId)
      console.log(res)
      setshowForm(false)
      fetchSubjects()
    ...
    <div className="card card-custom gutter-b">
       <div className="card-header">
          <div className="card-title">
             <h3 className="card-label">Class {match.params.name}</h3>
          </div>
          <div className="card-toolbar"><button onClick={()=>{setshowForm(true)}} type="button" className="btn btn-primary">New Subject</button></div>
       </div>
       <div className="card-header">
       ...
       <div className="card-body">
          ...
          <div className="react-bootstrap-table table-responsive">
             <table className="table table table-head-custom table-vertical-center overflow-hidden">
                ...
                <tbody>
                  {data && data.listOfSubjectId.map((classItem, index) => {
                     if(classItem.subjectId){
                        return <SubjectItem fetchSubjects={fetchSubjects} key={classItem.subjectId._id} data={classItem.subjectId} index={index}/>
                     }
                  })}
                </tbody>
             </table>
            ...
            <form action="#" className="form form-label-right">
              <div className="form-group row">
                <div className="col-lg-6">
                  <label>Subject Name</label><input type="text" ref={nameQ} className="form-control" name="className" placeholder="Subject name"  />
                  {/* <div className="feedback">Please enter <b>Class Name</b></div> */}
                </div>
                <div className="col-lg-6">
                  <label>Language</label><input type="text" ref={langQ} className="form-control" name="Language" placeholder="Language"  />
